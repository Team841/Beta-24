...
public class ComputeThread {
  ...
  public Supplier<Double> getHeading =
      () -> {
  ...
  public void compute() {
    ...
    while (running) {
      ...
      if (ConstantsIO.isRedAlliance.get()) { // Red side
        angleToFace =
            Math.atan(
                (Field.kRedSpeakerPose2d.getY() - this.mostRecentRobotPose.getY())
                    / (Field.kRedSpeakerPose2d.getX() - this.mostRecentRobotPose.getX()));
      } else { // blue side
        angleToFace =
            Math.atan(
                    (Field.kBlueSpeakerPose2d.getY() - this.mostRecentRobotPose.getY())
                        / (Field.kBlueSpeakerPose2d.getX() - this.mostRecentRobotPose.getX()))
                + 180;
      }

      this.ControllerOutput =
          HeadingController.calculate(
              this.mostRecentRobotPose.getRotation().getRadians(),
              angleToFace,
